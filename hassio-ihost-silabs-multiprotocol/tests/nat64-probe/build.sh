...
# Preserve the production feature/vendor configuration; only enable the two
# investigated capabilities. Empty CPCD_SOURCE_DIR selects installed libcpc.
...
cmake -S /usr/src/openthread -B /usr/src/unit-build -GNinja \
    ...
    -DOT_NAT64_TRANSLATOR=ON -DOT_NAT64_BORDER_ROUTING=ON
cmake --build /usr/src/unit-build --target ot-test-nat64 --parallel 2
ctest --test-dir /usr/src/unit-build --no-tests=error --output-on-failure -R '^ot-test-nat64$'
