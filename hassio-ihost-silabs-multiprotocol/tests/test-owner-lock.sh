...
python3 "${OWNER_LOCK}" --socket-name "${socket_name}" \
    -- "${TOUCH_COMMAND}" "${scratch}/post-release" 3>/dev/null
[[ -e "${scratch}/post-release" ]]

python3 "${OWNER_LOCK}" --socket-name "${socket_name}" \
    -- "${SLEEP_COMMAND}" 30 3>"${scratch}/killed-ready" &
holder_pid=$!
wait_for_readiness "${holder_pid}" "${scratch}/killed-ready"

set +e
python3 "${OWNER_LOCK}" --socket-name "${socket_name}" \
    -- "${TOUCH_COMMAND}" "${scratch}/blocked-during-command" \
    3>/dev/null 2>"${scratch}/contender.err"
contender_status=$?
set -e
[[ "${contender_status}" -eq 75 ]]
...
python3 "${OWNER_LOCK}" --socket-name "${socket_name}" \
    -- "${TOUCH_COMMAND}" "${scratch}/post-kill" 3>/dev/null
[[ -e "${scratch}/post-kill" ]]
