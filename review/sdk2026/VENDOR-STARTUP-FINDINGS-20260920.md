...
## Online evidence refresh after HFXO diagnostic — September 20

Operator direction: use relevant online evidence, documentation and existing work to avoid unnecessary implementation and experiments. Reuse sufficiently current research; refresh when the failure boundary, target version or proposed mechanism changes. Record applicability, not merely a list of links. This refresh made no production change.

- Rechecked [SDK2026.6.1 multiprotocol notes](https://docs.silabs.com/openthread/latest/sisdk-ot-release-notes/sisdk-ot-multiprotocol-release-notes) (page identifies version3.1.1/July29). Issue1209958 requires prolonged concurrent Zigbee/Thread/BLE traffic; it does not match silence before the first CPC response. The listed MG24 reconnect and newer-chip network-start fixes also do not establish an MG21 initial-startup fix.
- Rechecked [CPC4.9.1 notes](https://docs.silabs.com/gecko-platform/latest/sisdk-platform-release-notes/sisdk-plat-cpc-release-notes): no listed known issues; fixes concern multithreaded endpoint state and SPI compilation. This is not proof the UART startup path is defect-free.
- [Nabu Casa August18 beta](https://github.com/NabuCasa/silabs-firmware-builder/releases/tag/v2026.08.18-beta1) confirms SDK2026.6.1 adoption and supplies reusable build/patch work. Its Thread fixes include bounded flow-control transmission, buffering and watchdog behavior. These are candidates for later applicability review, not demonstrated fixes for this image: our link has no hardware flow control and fails before CPC communication. Do not add a watchdog now to conceal an unexplained startup stop. Existing pinned builder/source comparisons remain useful; this release does not supply a demonstrated ZBDongle-E multiprotocol replacement.
- [Vendor debugging guide](https://docs.silabs.com/openthread/latest/multiprotocol-solution-linux/debugging-local-processes) describes RCP logs through RTT or the host and warns excessive log output can exhaust resources and make the RCP unresponsive. Inference for our case: host-forwarded logs cannot be assumed available before the failing transport starts; RTT requires an accessible debug connection that has not been established. Keep targeted private serial tracing; before implementing full-image checkpoints, verify a usable output/retrieval path and reuse vendor facilities where practical.
- [Vendor CPC counter documentation](https://docs.silabs.com/gecko-platform/4.2/service/cpc/overview) describes debugger-accessible event/memory counters. This older reference is a reuse lead, not verification of the current generated component or a way to retrieve counters from our silent USB device. Check the retained exact SDK source only if pursuing this route.

Targeted public searches for `sli_aes_seed_mask`, RADIOAES hangs, SDK2026.6.1 MG21 boot failures, and community multiprotocol recipes did not establish a matching published workaround. Search indexing and inspected sources are limited. No basis was found to change bootloader/SE, disable encryption, or replace the reviewed crypto diagnostic with a claimed known fix. The prepared one-image crypto diagnostic and its approval boundary remain unchanged. If it produces no concrete correction, prioritize observable full-image startup evidence over further routine isolated-component outages, checking current vendor/community work first.
