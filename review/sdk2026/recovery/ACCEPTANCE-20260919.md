...
## September20 baseline and proposed off-bulb alternative

Read-only preflight at07:47UTC found the named bulb off but reporting fresh last-seen at07:46:49UTC. ALPSTUGA CO2 reported at07:47:04UTC. All relevant app versions and staged hashes match September19; see ../evidence/preflight-20260920.json.

If the operator explicitly approves this alternative, after the candidate restart and a fresh state check, turn only light.stue_loft_computerbord_bulb_01 on using its retained brightness/color, verify an explicit genOnOff/onOff read result, immediately turn it off and verify off by another explicit read. Keep the intervention brief; do not wait through the observation period with it on. Do not send brightness/color changes in this off-bulb alternative. If a concurrent user/automation change occurs, preserve the newer intent and report the interrupted test. If it is already on at test time, use the original one-step brightness change/restoration above. No plug switching. This is a proposed amendment, not authorization; the original rule against turning an off lamp on applies until the operator approves it.
