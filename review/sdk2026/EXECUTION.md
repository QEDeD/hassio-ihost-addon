...
Current status: the goal continuation requested work after the maintenance-window checkpoint. Offline preparation has resumed; completion of the HA service window has not been established, so no live HA action is underway. The service-window pause was a NEW explicit operator request, distinct from the older descriptive statement. All child agents are completed. Production flashing/cutover still requires concrete approval.

Before resuming live work, confirm the service window has ended and identify current HA versions, app states, serial mapping and fresh representative Zigbee/Thread reports. Treat maintenance-related changes as a new baseline; reuse completed offline evidence.
...
- CLI placement concern resolved by direct inspection of the manifest-pinned1.1.0 wheel: both the top-level parser and flash subparser inherit the same global parser. Existing before-subcommand syntax is supported by source. A small hardware-mocked parser test is saved at evidence/flasher-cli-parse.py but has NOT run: Docker's Linux-engine pipe was unavailable on resumption. Do not describe that additional test as passed. Prior real offline CLI/GBL checks remain valid.
...
Latest offline progress: all63 preserved flasher bundle files rehashed against the committed manifest; no mismatch. Docker is currently unavailable and was not restarted. No live HA access, image publication or production changes occurred during this continuation.
