...
Current status: paused for the operator-requested HA VM service window on 2026-09-19. This is a NEW, explicit maintenance-window pause, not the earlier misinterpreted descriptive statement. No HA operation was in flight when shutdown readiness was reported; this task made no production mutations. All child agents are completed. Main owns resumption and the remaining staging/approval procedure; token_compatibility also completed the timing recommendation below. Production flashing/cutover still requires concrete approval.

On operator resumption after the service window: first confirm HA has returned and identify its current versions, app states, serial mapping and fresh representative Zigbee/Thread reports. Treat maintenance-related changes as a new baseline; do not assume the pre-window runtime is unchanged. Then continue the remaining route below, reusing completed offline evidence.
...
- Before turning this rehearsal into a live command, reconcile actual installed CLI argument placement with TRIAL-PROCEDURE.md: captured flash help lists device/reset/probe options under the flash subcommand. The draft's global-before-subcommand form has not been validated; do not execute it as written merely because an earlier source review proposed it.
