...
def elf(path):
    ...
    return out,segments
source=srec(root/'input/rcp-uart-802154.s37'); e,segs=elf(root/'input/rcp-uart-802154.out')
assert e==source,'ELF/SREC differ'
parsed=srec(root/'output/parsed-application.s37')
b=(root/'output/rcp-sdk2026-application-only.gbl').read_bytes(); tags=[]; payload={}; off=0; app=None
while off<len(b):
...
assert hashlib.sha256((root/'input/rcp-uart-802154.out').read_bytes()).hexdigest()=='165a4a603d42eedd346b8e5e881662d41b9689eaecc762848d86535dfe064f74','Unexpected ELF input'
...
assert properties[28:56]==bytes.fromhex('02000000010000000000000000000000000000000000000000000000')
