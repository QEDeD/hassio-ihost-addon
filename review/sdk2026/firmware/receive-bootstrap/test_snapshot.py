...
def valid_record(build_id=0x52584231):
    values = [{"format_version":1, "build_id":build_id}.get(name,i) for i,name in enumerate(m.field_names())]
...
class Tests(unittest.TestCase):
    ...
    def test_previous_build_rejected_with_valid_crc(self):
        for previous in (0x504F5331, 0x52585331):
            with self.subTest(build_id=previous):
                with self.assertRaises(m.SnapshotError):
                    m.decode(valid_record(build_id=previous))
