...
def receive_overlay_driver(original):
...
def overlay_driver(original):
    modified = receive_overlay_driver(original)
    a, b, fn = function_span(modified, "static void restart_dma(void)")
    return modified[:a] + bootstrap_restart(fn) + modified[b:]
...
def prepare(root, sdk, recipe):
    ...
    manifest = {
        "purpose": "unproved bootstrap descriptor candidate with receive snapshot; no SDK mutation",
        "build_id": "52584231", "field_count": len(fields),
