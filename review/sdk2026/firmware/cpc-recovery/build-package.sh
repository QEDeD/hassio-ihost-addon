...
ELF_BASENAME=${ELF_BASENAME:-cpc_secondary_vcom_security_device_recovery}
[[ "$ELF_BASENAME" =~ ^[a-zA-Z0-9_-]+$ ]] || { echo 'Invalid ELF basename' >&2; exit 2; }
...
export EXPECTED_ELF_SHA256 GBL_NAME ELF_BASENAME
if [[ -e "$PACKAGE_WORK" ]]; then echo 'Refusing existing PACKAGE_WORK' >&2; exit 2; fi
mkdir -p "$PACKAGE_WORK/input" "$PACKAGE_WORK/output"
cp "$FIRMWARE_INPUT/$ELF_BASENAME.out" "$FIRMWARE_INPUT/$ELF_BASENAME.s37" "$PACKAGE_WORK/input/"
printf '%s  %s.out\n' "$EXPECTED_ELF_SHA256" "$ELF_BASENAME" | (cd "$PACKAGE_WORK/input"; sha256sum -c -)
...
"$COMMANDER" gbl3 create "$PACKAGE_WORK/output/$GBL_NAME" --app "$PACKAGE_WORK/input/$ELF_BASENAME.out" > "$PACKAGE_WORK/output/create.log"
