...
Still preparation. Remote image delivery/identities and the final representative control selection remain before requesting production approval. Post-reboot baseline and live flasher staging have passed; source-supported Zigbee read route resolves to zigbee2mqtt/stue_loft_computerbord_bulb_01/get. Confirm the installed exposes before that request. Bounded R1 timing and the completed independent review are incorporated. A successful build is not deployment readiness. The meaningful operator decision will be acceptance of the concrete single-dongle trial and its residual recovery risk, not mechanics we can resolve ourselves.
