...
- Host images: candidate0.3.1-sdk2026 ID sha256:f34bcbad0113df786aba81619b8ccf2b580bb1ac56ed8f3b3c23acc99c5c0844; recovery0.3.2-sdk2026-recovery-unbind ID sha256:048dda7c6872f102376dbd2a83aeaa8d93baa1300b42f4ba667cbd82f05cec27. Retained old host0.2.3-ordered is in recovery/REUSE-FINDINGS.md. Both are now published under ghcr.io/qeded/otbr-integration-test-amd64 with these exact index digests; AMD64 manifests and HA-side manifest verification are recorded in evidence/published-images-20260919.json. The image layers have not been pre-pulled by Supervisor; the stopped update will download them, within the same time budget.
...
## Completed preparation and immediate preflight
...
2. Registry publication is complete under explicit operator approval. HA fetched the exact manifests without private registry credentials. Descriptors staged, hash-verified and NOT activated: /share/codex-sdk2026-stage-20260919/candidate-config.json (6de7e93a17015c62f3195db59024e693bdc29cf0f3bb64944d27a8d51af50631) and recovery-config.json (6637c0f91c942abee7a4d1f3a41f9dfd7708f2bb6dab40f95d80f2ee0fccaaae). Their only difference from the tested build descriptors is the image field naming the approved registry. During an approved stopped update, preserve the current source descriptor privately, replace only /local_apps/codex_ihost_otbr_focused/config.json with the selected staged descriptor, reload the Supervisor local app store, verify the selected version, then perform the same-slug update. Do not widen SSH privileges or uninstall the app.
...
Reserve60minutes of attended radio interruption, with a decision to accept or begin recovery by T+35. This is an operational target, not a guarantee: failed bootloader/storage recovery can exceed the window. Initial stopped backup has a10-minute abort allowance before flash. No builds, dependency downloads or broad investigation during downtime. Only the exact approved registry image pulls are included in the same time budget. Complete the candidate host download/update before touching firmware; if it fails or leaves insufficient time for the remaining checkpoints, return to the baseline without flashing. Recovery may also require registry access: manifest retrieval has passed from HA, but cached image layers and future registry availability are not established. Do not describe this as fully offline recovery. See evidence/pretrial-registry-check-20260919.json.
...
Preparation is complete for an informed single-dongle trial decision. All approval artifacts are staged; registry digests and HA manifest retrieval match. The images are not installed, the dongle is unchanged, and fresh stopped-writer backups are intentionally part of the approved window immediately before flash. Recheck staged runtime, identities and current device responsiveness at that point.

Approval must cover the candidate flash and encrypted binding; named control checks; optional early R1 recovery with its one unbind/archival/rebind sequence; and at most one terminal old-firmware/host fallback as bounded above. Expected target is up to60minutes of attended radio interruption. A late recovery failure can exceed it, and network repair remains a residual possibility on the only dongle. No bootloader/SE flashing, mass erase, repeated candidate loops or unapproved plaintext fallback is included. If unable to continue within that scope, preserve state and request the next concrete decision.
